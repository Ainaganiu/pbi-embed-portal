...
function makeOpenAiCompatible({ name, apiKey, model, baseUrl }) {
  ...
  // endpoints would reject, so only send it there. Reasoning is switched off
  // deliberately: neither task here (write one DAX query, summarise a page of
  // visuals) benefits from it, while a reasoning model will happily spend its
  // entire token budget thinking and return nothing. Measured on this model:
  // 9.6s with reasoning vs 4.2s without, for an answer of the same length.
  const reasoningOff = /openrouter\.ai/i.test(baseUrl) ? { reasoning: { enabled: false } } : {};
  ...
  return {
    ...
    async completeStream({ system, messages, maxTokens }, onDelta) {
