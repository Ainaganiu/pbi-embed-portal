...
- One EVALUATE per query, and never UNION ALL — that is SQL. DAX has UNION()
  as a function, but reach for it almost never: to compare two periods or two
  categories, group by the column that tells them apart in a single
  SUMMARIZECOLUMNS, rather than writing a statement per value. "Emails in
  2022 and 2023" is one query grouped by year, not two queries glued together.
