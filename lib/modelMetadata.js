 ...
 * matching by name -- table+name for columns, since names alone aren't
 * unique across tables. Relationships match by table pair, which is the
 * finest key available on the INFO.VIEW side; when a table pair has more
 * than one relationship in the definition (e.g. a fact table linked to a
 * date dimension via two different date columns), which one owns which
 * crossFilteringBehavior can't be told apart at this key, so a colliding
 * pair gets no overlay at all rather than a guess that could tag the WRONG
 * relationship as bidirectional under the card's "exact" authority.
 *
 * A null definition returns metadata completely unchanged: a Fabric-fetch
 * failure alongside a successful INFO.VIEW read must look exactly like a
 * sync that never had this feature.
 */
function mergeDefinition(metadata, definition) {
  ...
  for (const r of definition.relationships) {
    const key = `${r.fromTable}::${r.toTable}`;
    relCounts.set(key, (relCounts.get(key) || 0) + 1);
  }
  const relBehavior = new Map(
    definition.relationships
      .filter((r) => relCounts.get(`${r.fromTable}::${r.toTable}`) === 1)
      .map((r) => [`${r.fromTable}::${r.toTable}`, r.crossFilteringBehavior])
  );
