...
async function updateReport(id, data) {
  // A report repointed at a different dataset no longer describes the model
  // that was last synced -- the old model's exact table/measure/column names
  // would otherwise keep being served to chat, stamped "read from the
  // semantic model itself... use them verbatim" for a dataset they no longer
  // belong to. Read the current dataset_id first so a same-dataset save (the
  // common case) leaves a fresh sync untouched, per Task 2's design.
  const existing = await pool.query("SELECT dataset_id FROM reports WHERE id = $1", [id]);
  const newDatasetId = data.datasetId || null;
  const datasetChanged =
    existing.rows.length > 0 && existing.rows[0].dataset_id !== newDatasetId;

  const clearMetadataSql = datasetChanged
    ? ", model_metadata = NULL, model_metadata_synced_at = NULL"
    : "";
  ...
  await pool.query(
    `UPDATE reports
     SET name = $2, workspace_id = $3, report_id = $4, dataset_id = $5,
         ...
         measures_description = $8, columns_description = $9, sort_order = $10${clearMetadataSql}
     WHERE id = $1`,
    [
      id,
      data.name,
