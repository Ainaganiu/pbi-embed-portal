...
async function run(ctx, emit) {
  ...
  // The prior turns are folded into the cache key itself now, so a genuine
  // repeat of the same conversation still hits and an unrelated one asking
  // the same words still misses -- see lib/llmCache.js.
  ...
  // The latest generation's hint wins -- see the identical comment on
  // screen.js's escalation, which shares this rule.
  let format = null;

  // Delegates to the shared generateDaxFor (lib/answer/dax.js) rather than
  // keeping a second copy of this logic -- a duplicate here is how
  // reasoning and lintDax's mechanical repairs drifted out of sync with the
  // screen path in the first place. reasoning defaults to "low": a period
  // or filter named in the question is exactly the case a plausible-looking
  // wrong query reads as an answer, which is what screen.js's escalation
  // already always asks for.
  async function generateDax(messages, opts = {}) {
    const result = await generateDaxFor(report, provider, messages, { reasoning: "low", ...opts });
      ...
      const retryMessages = [
        ...
        { role: "assistant", content: firstDax },
        {
          ...
          content:
            ...
            `${firstErr.message}\n\n` +
            `Return a corrected DAX query, keeping the same filters, ` +
            `grouping and values named in the question — fix only what the ` +
            `engine rejected. Check table-name quoting, that CALCULATE ` +
            `filters are simple column comparisons, and that every column, ` +
            `measure and filter value exists in the schema exactly as ` +
            `written. Return ONLY the query.`,
        },
      ];
      dax = await generateDax(retryMessages, { allowClarify: false });

      // A syntax fix can "succeed" by dropping the very filter that was
      // rejected, rather than repairing it -- exactly the failure the
      // pre-execution grounding check above exists to catch, checked again
      // here since this query was written after that check already ran.
      try {
        const retryIssues = groundingIssues(question, dax);
        if (retryIssues.length) {
          console.error("[grounding] retry dropped grounding:", retryIssues.join("; "));
          dax = await generateDax(
            [
              ...retryMessages,
              { role: "assistant", content: dax },
              {
                role: "user",
                content:
                  `That still does not answer the question: ${retryIssues.join("; ")}.\n\n` +
                  `Return ONLY a corrected query that both runs and keeps that filter.`,
              },
            ],
            { allowClarify: false }
          );
        ...
        console.error("[grounding] retry re-check failed:", err.message);
      }
  ...
  try {
    const raw = await provider.complete({
      system:
        ...
        `Earlier turns count: read them for what was already asked and said, ` +
        `so a bare follow-up like "and 2023?" reads as a continuation of that ` +
        `same thread rather than a fresh, disconnected answer.\n\n` +
        GROUNDING_RULE +
        `\nWhat the underlying model actually contains, for reference:\n` +
        `${schemaContext(report)}`,
      messages: [
        ...priorTurns,
        {
          role: "user",
    ...
    chart = buildChartSpec(ctx.question, rows, { intent: parsed.intent, format });
