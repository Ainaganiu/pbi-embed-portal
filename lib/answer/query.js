...
async function run(ctx, emit) {
  ...
  async function generateDax(messages) {
    ...
    // A clarify payload carries no format hint, and stripping a marker line
    // out of it would corrupt the JSON the caller still needs to parse.
    if (findClarify(raw)) return raw;

    const split = splitFormatHint(raw);
    format = split.format;
    return split.dax;
  }
