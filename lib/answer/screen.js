...
async function run(ctx, emit) {
  ...
  let queryError = null;
  let usedDax = null;
  // The latest generation's hint wins -- a correction that changes what's
  // being computed should carry its own FORMAT line, and one that doesn't
  // need a percentage simply omits it, clearing any earlier hint.
  let usedFormat = null;
  try {
    if (ctx.aborted()) throw new Error("cancelled");
    ({ dax: usedDax, format: usedFormat } = await generateDaxFor(
      report,
      provider,
  ...
  const chartSpec = queryError ? null : buildChartSpec(question, rows || [], { format: usedFormat });
