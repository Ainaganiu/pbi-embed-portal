...
function renderReportState(state, focusVisualName) {
  ...
  // The router is shown more visuals than fit in this prompt, so it can
  // legitimately focus one that falls outside the budget. Slicing by position
  // first dropped exactly that visual — the one the question was about — and
  // said nothing about it, so the focus is hoisted in before the cut.
  const focusIndex = all.findIndex(isFocus);
  const ordered =
    focusIndex >= BUDGETS.VISUALS_IN_PROMPT
      ? [all[focusIndex], ...all.filter((_, i) => i !== focusIndex)]
      : all;
  const visuals = ordered.slice(0, BUDGETS.VISUALS_IN_PROMPT);
  ...
  if (all.length > visuals.length) {
    lines.push(`\n(${all.length - visuals.length} further visuals omitted.)`);
  }
