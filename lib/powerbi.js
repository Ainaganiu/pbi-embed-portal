...
async function pbiFetch(credentials, path, options = {}) {
  ...
  if (!res.ok) {
    throw new Error(
      `Power BI API ${options.method || "GET"} ${path} failed: ${res.status} ${await readErrorBody(res)}`
    );
  }
...
module.exports = { getAadToken, clearTokenCache, generateEmbedToken, executeQuery };
