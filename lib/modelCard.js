...
function render(metadata, report) {
  ...
  // A single real DAX formula can be arbitrarily long; capping it keeps one
  // measure from consuming a whole section's budget by itself. Continuation
  // lines of a multi-line expression are indented like the rest of the
  // entry, not left at column 0, where they would read as a new top-level
  // card line rather than part of the formula above them.
  const MAX_EXPRESSION_CHARS = 600;
  const expressionLines = (expression) => {
    if (!expression) return [];
    const clipped =
      expression.length > MAX_EXPRESSION_CHARS ? `${expression.slice(0, MAX_EXPRESSION_CHARS)}…` : expression;
    const [first, ...rest] = clipped.split("\n");
    return [`      = ${first}`, ...rest.map((l) => `      ${l}`)];
  };

  // MEASURES and COLUMNS each give way independently once the running total
  // would blow the budget -- a measure-heavy model must not silently
  // swallow every column (or vice versa). The card states how many entries
  // of THAT section it dropped, the same way COLUMNS always has, rather
  // than ending mid-formula with no notice that anything is missing.
  const renderSection = (title, items, kind, noun) => {
    if (!items?.length) return;
    const entryLines = [];
    let omitted = 0;
    for (const item of items) {
      const obj = { kind, name: item.name, table: item.table };
      const entry = [`  ${qualified(obj)}  ${describeType(item)}`.trimEnd(), ...expressionLines(item.expression)];
      ...
      const runningLength = out.join("\n").length + entryLines.join("\n").length;
      if (runningLength + addition > BUDGETS.MODEL_CARD_CHARS - 200) {
        omitted += 1;
        continue;
      }
    ...
    if (omitted) out.push(`  (${omitted} further ${noun} omitted.)`);
  };

  renderSection("MEASURES", metadata.measures, "measure", "measures");
  renderSection("COLUMNS", metadata.columns, "column", "columns");
