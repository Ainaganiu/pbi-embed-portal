 ...
 */
function buildChartSpec(question, rows, caption) {
  const data = Array.isArray(rows) ? rows.filter(Boolean) : [];
  const choice = chooseChartType(question, data);
  ...
  if (choice.type === "card") {
    return { type: "card", values: [data[0][numeric[0]]], label };
  }

  if (choice.type === "table") {
    // Every column, in the order the query returned them — for a table the
    // values themselves are the point, so nothing is dropped.
  ...
  const labels = data.map((r) => String(r[category]));
...
module.exports = { chooseChartType, buildChartSpec, classify, isTemporal, prettyName };
