...
// The categorical columns that actually tell one row from another.
//
// A query grouped by two columns comes back as a cross-tab: "Calls" appears
// once per issue type, and only the second column says which is which. A
// column holding the same value in every row is a different case -- grouping
// by Year and Genre while filtered to one year leaves Year constant, so it
// distinguishes nothing and the chart should still be a bar by genre.
function distinguishingDimensions(rows, categorical) {
  return categorical.filter((key) => {
    const seen = new Set();
    for (const row of rows) {
      const v = row?.[key];
...
function chooseChartType(question, rows, intent) {
  ...
  const dimensions = distinguishingDimensions(data, categorical);

  // Two dimensions is a cross-tab, and no flat chart can show one honestly:
  // plotting the first alone repeats "Calls" once per issue type, each bar a
  // different number with the same label and nothing saying why. The table
  // shows both columns, so it is the only form that answers the question
  // that was actually asked.
  if (dimensions.length >= 2) {
    return {
      type: "table",
      reason: `grouped by ${dimensions.length} columns`,
      fixed: true,
    };
  }

  const category =
    dimensions.find((key) => isTemporal(key, data)) ||
    dimensions[0] ||
    categorical.find((key) => isTemporal(key, data)) ||
    categorical[0];
...
function buildChartSpec(question, rows, options = {}) {
  ...
  const { numeric, categorical } = classify(data);
  const dimensions = distinguishingDimensions(data, categorical);
  // Same precedence as chooseChartType: plot the column that actually varies,
  // so a constant Year column can't end up labelling every bar "2016".
  const category =
    dimensions.find((key) => isTemporal(key, data)) ||
    dimensions[0] ||
    categorical.find((key) => isTemporal(key, data)) ||
  ...
  if (choice.type === "table") {
    ...
    // values themselves are the point, so nothing is dropped. It used to
    // keep one categorical column and drop the rest, which is what made a
    // two-dimension result unreadable: four interaction types repeating down
    // the page with the issue type that told them apart missing entirely.
    //
    // A table can mix a percentage measure with an ordinary one, so a single
    // spec-level format flag would misdescribe half its columns -- percent
    // formatting is deliberately not applied here.
...
function validTypesFor(rows) {
  ...
  const dimensions = distinguishingDimensions(data, categorical);
  // A cross-tab has no honest chart form, so the switcher must not offer one
  // -- picking a bar there would flatten the second dimension back out and
  // relabel the rows exactly as ambiguously as before.
