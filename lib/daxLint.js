...
// SQL habits that DAX does not share. UNION ALL is the one that actually
// happened: asked for two years, the model wrote a ROW for each and joined
// them with an operator that does not exist here. DAX has UNION() as a
// function, and this endpoint runs exactly one EVALUATE per query, so the
// shape is doubly wrong -- and a single grouped query is the better answer
// anyway, since it gives the years a column to sit on.
const SQL_UNION = /\bUNION\s+ALL\b/i;
const EVALUATE_STATEMENT = /\bEVALUATE\b/gi;
...
function groundingIssues(question, dax) {
  ...
  if (SQL_UNION.test(d)) {
    issues.push(
      "the query uses UNION ALL, which is SQL and not valid DAX — return ONE " +
        "EVALUATE that groups by the column distinguishing the cases, rather " +
        "than one statement per case"
    );
  }

  const evaluates = (d.match(EVALUATE_STATEMENT) || []).length;
  if (evaluates > 1) {
    issues.push(
      `the query has ${evaluates} EVALUATE statements and this endpoint runs ` +
        `one — return a single EVALUATE, grouping by the column that ` +
        `distinguishes the cases`
    );
  }

  // Two periods named in one question is a comparison even when it says
  // neither "compare" nor "by": "emails in 2022 and 2023" wants a row per
  // year, not a query per year.
  const comparesPeriods = years.length >= 2;

  if ((IMPLIES_BREAKDOWN.test(q) || comparesPeriods) && !GROUPS_RESULTS.test(d)) {
    issues.push(
